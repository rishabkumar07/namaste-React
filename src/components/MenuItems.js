...
const MenuItems = ({items}) => {
  ...
  return (
    <div>
      {items.map((item)=> {
        return (
          <div data-testid="foodItems" key={item.card.info.id} className="p-2 m-2 border-gray-200 border-b-2 
            ...
            <div className="w-3/12 p-4 relative">
              <div>
                <button className="p-0 mx-16 rounded-lg bg-black text-white shadow-lg
                  absolute bottom-4 left-5 w-12 h-6 text-sm" onClick={() => handleClick(item)} >
                  Add +
                </button>
              </div>
